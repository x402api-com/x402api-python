...
[**charges_submit_payment**](ProgrammaticChargesApi.md#charges_submit_payment) | **POST** /v1/charges/{charge_id}/payments | Submit a programmatic charge payment
...
# **charges_submit_payment**
> DynamicChargePaymentResponse charges_submit_payment(payment_signature, charge_id)
...
with x402api.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = x402api.ProgrammaticChargesApi(api_client)
    payment_signature = 'payment_signature_example' # str | Canonical base64-encoded x402 v2 PaymentPayload.
    charge_id = UUID('38400000-8cf0-11bd-b23e-10b96e4ef00d') # UUID |

    try:
        # Submit a programmatic charge payment
        api_response = api_instance.charges_submit_payment(payment_signature, charge_id)
        print("The response of ProgrammaticChargesApi->charges_submit_payment:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ProgrammaticChargesApi->charges_submit_payment: %s\n" % e)
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **payment_signature** | **str**| Canonical base64-encoded x402 v2 PaymentPayload. |
 **charge_id** | **UUID**|  |
